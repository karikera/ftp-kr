...
function createClient(
	...
	let newclient: FileInterface;
	switch (config.protocol) {
		case 'sftp':
			newclient = new SftpConnection(workspace, config);
			break;
		case 'ftp':
			newclient = new FtpConnection(workspace, config);
			break;
		case 'ftps':
			newclient = new FtpConnection(workspace, config);
			break;
		default:
			throw Error(`Invalid protocol ${config.protocol}`);
	}
	return newclient;
}
...
export class FtpManager {
	...
	private _blockTestWrap<T>(
		task: Task,
		callback: (client: FileInterface) => Promise<T>
	) {
		return promiseErrorWrap(
			this.connect(task).then(async (client) => {
				let tryCount = 0;
				for (;;) {
					...
					try {
						...
						if (err.ftpCode === FtpErrorCode.REUQEST_RECONNECT_AND_RETRY_ONCE) {
							...
							err.ftpCode === FtpErrorCode.REUQEST_RECONNECT_AND_RETRY
						) {
							this.terminate();
							client = await this.connect(task);
						} else {
	...
	public async connect(task: Task): Promise<FileInterface> {
		...
		const tryToConnect = async (
			...
			try {
				for (;;) {
					const client = createClient(this.workspace, config);
					try {
						this.logger.message(
							`Trying to connect to ${config.url} with user ${config.username}`
						);
						await this._blockTestWith(task, client.connect(password));
		...
		this.client.oninvalidencoding = (errfiles: string[]) => {
			this.logger
				.errorConfirm(
					'Invalid encoding detected. Please set fileNameEncoding correctly\n' +
						errfiles.join('\n'),
					'Open config',
					'Ignore after'
				)
				.then((res) => {
					switch (res) {
						case 'Open config':
							vsutil.open(this.workspace.query(Config).path);
							break;
						case 'Ignore after':
							return this.workspace
								.query(Config)
								.modifySave((cfg) => (cfg.ignoreWrongFileEncoding = true));
					}
				});
		};
	...
	public upload(task: Task, ftppath: string, localpath: File): Promise<void> {
		return this._blockTestWrap(task, (client) =>
			client.upload(ftppath, localpath)
		);
	}

	public download(task: Task, localpath: File, ftppath: string): Promise<void> {
		return this._blockTestWrap(task, (client) =>
			client.download(localpath, ftppath)
		);
	}
	...
	public readlink(
		...
		return this._blockTestWrap(task, (client) =>
			client.readlink(fileinfo, ftppath)
		);
	}
	...
	public rename(
		...
		return this._blockTestWrap(task, (client) =>
			client.rename(ftppathFrom, ftppathTo)
		);
	}
}
