...
export class FtpSyncManager implements WorkspaceItem {
	...
	public async onLoadConfig(): Promise<void> {
		let targetServerIndex = this.targetServer
		...
		ftpTree.refreshTree();
		this.targetServer =
			this._getServerFromIndex(targetServerIndex) || mainServer;
	}
	...
	public async selectServer(
		openAlways?: boolean
	): Promise<FtpCacher | undefined> {
		let selected: FtpCacher | undefined = undefined;
		...
		for (const server of this.servers.values()) {
			...
			pick.item(name, () => {
				selected = this.servers.get(config);
			});
		}
	...
	public reconnect(task?: Task | null): Promise<void> {
		return this.scheduler.taskMust(
			'Reconnect',
			(task) => {
	...
	public async runTaskJson(
		...
		await selected.runTaskJsonWithConfirm(
			taskName,
			tasks,
			taskjson.basename(),
			options
