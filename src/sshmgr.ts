...
function getShellType(): string | undefined {
	...
	let shellPath: string | undefined = terminalSettings.integrated.shell.windows;
...
export function openSshTerminal(server: FtpCacher): void {
	...
	let dir = server.workspace.fsPath;
	switch (getShellType()) {
		case 'wslbash':
	...
	if (server.config.protocol !== 'sftp') {
		server.logger
			.errorConfirm(
				'Cannot open SSH. Need to set protocol to sftp in ftp-kr.json',
				'Open config'
			)
			.then((res) => {
				switch (res) {
					case 'Open config':
						vsutil.open(server.workspace.query(Config).path);
						break;
				}
			});
		return;
