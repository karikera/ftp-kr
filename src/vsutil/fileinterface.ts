...
export abstract class FileInterface {
	...
	_callWithName<T>(
		...
		return promiseErrorWrap(
			callback(this.str2bin(ftppath)).then(
				(v) => {
					this.stateBar.close();
					return v;
				},
		...
		return promiseErrorWrap(
			this._put(localpath, binpath).then(
				...
				(err) => {
					...
					this.log(
						`upload fail: ${ftppath}, ${
							errorMessageOverride || err.message || err
						}`
					);
					throw err;
				}
	...
	download(localpath: File, ftppath: string): Promise<void> {
		...
		return promiseErrorWrap(
			this._get(this.str2bin(ftppath)).then(
				(stream) => {
	...
	write(ftppath: string, content: Buffer): Promise<void> {
		...
		return promiseErrorWrap(
			this._write(content, this.str2bin(ftppath)).then(
				() => {
					this.stateBar.close();
				},
				(err) => {
	...
	list(ftppath: string): Promise<FileInfo[]> {
		...
		return promiseErrorWrap(
			this._list(this.str2bin(ftppath)).then(
				(list) => {
					this.stateBar.close();
	...
	rename(ftppathFrom: string, ftppathTo: string): Promise<void> {
		...
		return promiseErrorWrap(
			this._rename(this.str2bin(ftppathFrom), this.str2bin(ftppathTo)).then(
				() => {
					this.stateBar.close();
				},
				(err) => {
					this.stateBar.close();
					this.log(
						`rename fail: ${ftppathFrom} ${ftppathTo}, ${err.message || err}`
					);
					throw err;
				}
			)
