...
class FtpClient extends FtpClientO {
	...
	public list(
		path:
			| string
			| boolean
			| ((error: Error, listing: FtpClientO.ListingElement[]) => void),
		zcomp?:
			| boolean
			| ((error: Error, listing: FtpClientO.ListingElement[]) => void),
		cb?: (error: Error, listing: FtpClientO.ListingElement[]) => void
	): void {
		let pathcmd: string;
		...
		if (path.indexOf(' ') === -1) return super.list(pathcmd, zcomp, cb);
	...
	public terminate() {
		...
		if (anythis._pasvSock) {
			if (anythis._pasvSock.writable) anythis._pasvSock.destroy();
			anythis._pasvSock = undefined;
		}
		if (anythis._socket) {
			if (anythis._socket.writable) anythis._socket.destroy();
...
export class FtpConnection extends FileInterface {
	...
	async _connect(password?: string): Promise<void> {
		try {
			...
			let options: FtpClientO.Options;
			...
			return await new Promise<void>((resolve, reject) => {
				client
					.on('ready', () => {
						...
						socket.write = (str: string) =>
							oldwrite.call(socket, str, 'binary' as any); // XXX: TS bug, overloading is not considered.
	...
	static wrapToPromise<T>(
		callback: (cb: (err: Error, val: T) => void) => void
	): Promise<T> {
	...
	_delete(ftppath: string): Promise<void> {
		...
		if (!client) return Promise.reject(Error(NOT_CREATED));
		return FtpConnection.wrapToPromise<void>((callback) =>
			client.delete(ftppath, callback)
		).catch((e) => {
	...
	_put(localpath: File, ftppath: string): Promise<void> {
		...
		if (!client) return Promise.reject(Error(NOT_CREATED));
		return FtpConnection.wrapToPromise<void>((callback) =>
			client.put(localpath.fsPath, ftppath, callback)
		).catch((err) => {
			if (err.code === 'ECONNRESET')
				err.ftpCode = FtpErrorCode.REUQEST_RECONNECT_AND_RETRY_ONCE;
	...
	_write(buffer: Buffer, ftppath: string): Promise<void> {
		...
		if (!client) return Promise.reject(Error(NOT_CREATED));
		return FtpConnection.wrapToPromise<void>((callback) =>
			client.put(buffer, ftppath, callback)
		).catch((err) => {
			if (err.code === 'ECONNRESET')
				err.ftpCode = FtpErrorCode.REUQEST_RECONNECT_AND_RETRY_ONCE;
			else if (err.code === 451) err.ftpCode = FtpErrorCode.REQUEST_MKDIR;
	...
	_get(ftppath: string): Promise<NodeJS.ReadableStream> {
		const client = this.client;
		if (!client) return Promise.reject(Error(NOT_CREATED));
		return FtpConnection.wrapToPromise<NodeJS.ReadableStream>((callback) =>
			client.get(ftppath, callback)
		).catch((err) => {
			if (err.code === 'ECONNRESET')
				err.ftpCode = FtpErrorCode.REUQEST_RECONNECT_AND_RETRY_ONCE;
			else if (err.code === 550) err.ftpCode = FtpErrorCode.FILE_NOT_FOUND;
	...
	_list(ftppath: string): Promise<FileInfo[]> {
		const client = this.client;
		if (!client) return Promise.reject(Error(NOT_CREATED));
		return FtpConnection.wrapToPromise<FtpClientO.ListingElement[]>(
			(callback) => client.list(ftppath, false, callback)
		).then(
			(list) =>
				list.map((from) => {
	...
	_rename(ftppathFrom: string, ftppathTo: string): Promise<void> {
		...
		if (!client) return Promise.reject(Error(NOT_CREATED));
		return FtpConnection.wrapToPromise<void>((callback) =>
			client.rename(ftppathFrom, ftppathTo, callback)
		).catch((err) => {
			if (err.code === 'ECONNRESET')
				err.ftpCode = FtpErrorCode.REUQEST_RECONNECT_AND_RETRY_ONCE;
			throw err;
