...
import { commands, Disposable, TextDocument, Uri, window, workspace } from "vscode";
...
export class TemporalDocument {
    ...
    constructor(public readonly editorFile:File, public readonly targetFile:File) {
        this.editorFileUri = Uri.file(this.editorFile.fsPath);
        this.onClose = new Promise(resolve=>listen(this.editorFileUri.fsPath, ()=>{
            this._close();
            resolve();
        }));
    }
